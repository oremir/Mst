...
Mst.ShowPlayerItems = function (game_state, name, position, properties) {
    "use strict";
...
Mst.ShowPlayerItems.prototype = Object.create(Mst.ShowStat.prototype);
Mst.ShowPlayerItems.prototype.constructor = Mst.ShowPlayerItems;

Mst.ShowPlayerItems.prototype.show_initial_stats = function () {
    "use strict";
...
Mst.ShowPlayerItems.prototype.reset = function (position_x, position_y) {
    "use strict";
...
Mst.ShowPlayerItems.prototype.update_stat = function (new_stat) {
    "use strict";
...
Mst.ShowPlayerItems.prototype.create_new_stat_sprite = function (stat_index, frame, quantity) {
    "use strict";
    ...
    if (stat) {
        ...
        // stat sprite uses the same texture as the ShowPlayerItems prefab
...
Mst.ShowPlayerItems.prototype.put_down_item = function (one_item) {
    var item_index, item_quantity, chest_item;
    // Player
    ...
    chest_item = this.game_state.prefabs.chestitems.index_by_frame(one_item.frame);
    item_index = chest_item.index;
    
    if (chest_item.is_in) {
        item_quantity = chest_item.quantity;
        item_quantity++;
        this.game_state.game.prefabs.chest.texts[item_index].text = item_quantity;
    } else {
        item_index = 0;
        item_quantity = 1;
    }
    
    this.game_state.game.prefabs.chest.update_item(item_index, one_item.frame, item_quantity);
...
Mst.ShowPlayerItems.prototype.update_item = function (item_index, item_frame, item_quantity) {
    var items_packed, items_packed_splited, item_updated, is_in_items;
    is_in_items = true;
    
    item_updated = item_frame + "-" + item_quantity;

    items_packed = this.game_state.prefabs.player.stats.items;
    items_packed_splited = items_packed.split("_");
        ...
        if (items_packed_splited[item_index].split("-")[0] == item_frame) {
            ...
            for (var i = 0; i < items_packed_splited.length; i++) {
                if (items_packed_splited[i].split("-")[0] == item_frame) {
                    items_packed_splited[i] = item_updated; 
                    is_in_items = true;
                }
            }
        }
    ...
    this.game_state.prefabs.player.stats.items = items_packed_splited.join("_");
...
Mst.ShowPlayerItems.prototype.index_by_frame = function (item_frame) {
